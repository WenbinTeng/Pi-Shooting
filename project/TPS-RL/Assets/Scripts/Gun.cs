...
public class Gun : MonoBehaviour
{
    ...
    private void AimFunc()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            {
                Vector3 lookDirect = new Vector3(transform.parent.position.x, transform.parent.position.y, hit.point.z - 0.5f);
                transform.parent.rotation = Quaternion.Lerp(transform.parent.rotation, Quaternion.LookRotation(lookDirect - transform.parent.position), 0.2f);

                Vector3 lookRotate = new Vector3(transform.position.x, hit.point.y - 0.5f, hit.point.z - 0.5f);
                transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.LookRotation(lookRotate - transform.position), 0.2f);
            }
        }
    }
